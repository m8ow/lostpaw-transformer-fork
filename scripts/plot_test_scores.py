"""
学習メトリクスの可視化とプロット作成ユーティリティ

このスクリプトは、損失曲線、精度プロット、その他の性能指標を含む学習メトリクスの
論文品質のプロットを生成します。スムージング、信頼区間、様々な出力形式をサポートします。

主な機能:
1. 信頼区間付きのスムージングされたメトリクス可視化
2. 複数の出力形式（PDF、PNG等）
3. カスタマイズ可能なプロットスタイルとパラメータ
4. 学習実行の統計解析
5. クロスバリデーション結果の集約

使用方法:
    python plot_test_scores.py
    (スクリプト内でデータソースとプロットパラメータを設定)
"""
